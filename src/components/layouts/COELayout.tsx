...
import {
  LayoutDashboard, Upload, FileCheck, LogOut, Moon, Sun, ClipboardList,
} from 'lucide-react';
...
const navItems = [
  { icon: LayoutDashboard, label: 'Dashboard', path: '/coe' },
  { icon: Upload, label: 'Upload Results', path: '/coe/upload' },
  { icon: FileCheck, label: 'Published Results', path: '/coe/published' },
];

export function COELayout({ children }: { children: ReactNode }) {
    ...
    <aside className="h-full flex flex-col relative overflow-hidden" style={{ background: 'linear-gradient(180deg, hsl(230 28% 6%) 0%, hsl(230 30% 10%) 100%)' }}>
      ...
      <div className="p-4 border-b border-border/20 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display font-semibold text-xs tracking-wider text-primary-foreground">COE Portal</h2>
            <p className="text-[10px] text-muted-foreground font-body tracking-wider">Examination Cell</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-border/20 relative z-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-primary to-neon-pink flex items-center justify-center">
            <span className="text-sm font-bold text-primary-foreground font-display">C</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-body font-medium truncate text-primary-foreground">COE Cell</p>
            <p className="text-[10px] text-muted-foreground font-body truncate">{user?.email}</p>
          </div>
        </div>
        <div className="flex gap-1.5">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary-foreground hover:bg-primary-foreground/5"
            onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
            <Sun className="h-3.5 w-3.5 rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0" />
            <Moon className="absolute h-3.5 w-3.5 rotate-90 scale-0 transition-all dark:rotate-0 dark:scale-100" />
          </Button>
          <Button variant="ghost" className="flex-1 justify-start gap-2 text-xs text-muted-foreground hover:text-destructive hover:bg-destructive/10 h-8 font-body"
